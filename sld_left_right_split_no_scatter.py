...
class SlowLightDeviceWithLeftRightSplitNoScatter:
    """
    In this class we split moving forward and backwards to 2 different states, doubling the number of states
    to O(2*N). This will allow for decreasing d_min more without going over a probability of 1.
    We also disable scattering here entirely (setting alpha to 0 isn't enough, as we get an absorbing state with 0
    probability to reach and that makes the Q matrix singular.
    """

    def __init__(self,
                 gamma: float = 10,  # was: 100,
                 ...
                 L: float = 1.,
                 min_d: float = 0.,  # was: 0.01,
                 min_L: float = 0.,  # was: 0.01,
                 L_init: list = None,
                 d_init: list = None,
                 lr: float = 1e-5,
        ...
        self.chain, self.Pt, self.num_states, self.init_index, self.finish_index, \
    ...
    def check_params(self):
        ...
        return (total_length_check > 0) and (max_move_prob <= 1)
    ...
    def initialize_P(self, Lt: torch.Tensor, dt: torch.Tensor):
        num_states = self.N * 2 + 3  # 2*N transition states + init state + death by return to init + death by decay +
        # finish
        init_index = 0
        finish_index = 2 * self.N + 1  # in the R matrix this is index 0
        return_to_start_index = 2 * self.N + 2  # in the R matrix this is index 1
        ...
        reflect_forward_to_backward_probs = 1 - forward_probs
        reflect_backward_to_forward_probs = 1 - backward_probs
        ...
        chain = AbsorbingMarkovChain(P, num_absorbing=2, init_calculated_matrices=True)

        return chain, P, num_states, init_index, finish_index, return_to_start_index, \
    ...
    def _p_reflect(chain: AbsorbingMarkovChain):
        return chain.probability_of_absorption(0, 1)  # in the R matrix, reflecting is index 1
    ...
    def _build_loss(self, Lt: torch.Tensor, chain: AbsorbingMarkovChain):
        ...
        loss = time_loss + self.weight_of_p_finish_loss * finish_loss
        # print(loss)
    ...
    def pure_monte_carlo_search_device(self, num_steps: int, draw_best_device: bool = True):
        # Note this compares a total of num_steps * self.batch_size random devices
        assert self.is_optimize, "Not supported in non-optimizing mode."
        self.train_start_time = time.time()
        self.log(0, draw_best_device=draw_best_device)
        best_loss = np.inf
        for i in range(num_steps):
            self.Lt, self.dt = self.initialize_lengths()
            self.chain, self.Pt, self.num_states, self.init_index, self.finish_index, \
            self.return_to_start_index, self.forward_slab_indices, self.backward_slab_indices = \
                self.initialize_P(self.Lt, self.dt)
            loss = self.build_loss().detach().cpu().numpy()
            step_best_loss = loss.min()
            if step_best_loss < best_loss:
                best_loss = step_best_loss
                best_sample_ind = loss.argmin()
                best_Lt = self.Lt[best_sample_ind].detach().cpu().numpy()
                best_dt = self.dt[best_sample_ind].detach().cpu().numpy()
                self.log(i + 1, draw_best_device=draw_best_device)
...
if __name__ == '__main__':
    ...
    for i in range(redos):
        seed = init_seed + i
        for finish_prob in np.arange(0, 1.05, 0.05):
            sld = SlowLightDeviceWithLeftRightSplitNoScatter(seed=seed,
                                                             batch_size=256,
                                                             device=device,
                                                             N=5,
                                                             min_L=0,
                                                             ...
                                                             log_dir_suffix=f"after_fix_seed_{seed}__pf_"
                                                                            f"{finish_prob:.2f}")
        ...
        for N in range(1, 15 + 1):
            sld = SlowLightDeviceWithLeftRightSplitNoScatter(seed=seed,
                                                             ...
                                                             gamma=20,
                                                             p_finish_wanted=0.1,
                                                             log_dir_suffix=f"after_fix_seed_{seed}__N_{N}")
            sld.train(20000, 100, draw_best_device=False)

        for gamma in np.logspace(-1, np.log10(40), 21):
            try:
                sld = SlowLightDeviceWithLeftRightSplitNoScatter(seed=seed,
                                                                 batch_size=256,
                                                                 device=device,
                                                                 ...
                                                                 gamma=gamma,
                                                                 p_finish_wanted=0.1,
                                                                 log_dir_suffix=f"after_fix_seed_{seed}__gamma_{gamma}")
                sld.train(20000, 100, draw_best_device=False)
            # for when gamma is too large and stuff get messy
            except AssertionError:
                print("LARGE GAMMA ERROR!!")

    ##############
    # Single run:
    ##############
    # sld = SlowLightDeviceWithLeftRightSplitNoScatter(seed=73, batch_size=1024, device=device, N=5,
    #                                                  p_finish_wanted=0.1,
    #                                                  gamma=20,
    #                                                  log_dir_suffix=f"initial_results_after_fix")
    # sld.train(50000, 10, draw_best_device=False)
    # sld.pure_monte_carlo_search_device(num_steps=0)
